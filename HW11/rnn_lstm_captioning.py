...
def rnn_step_backward(dnext_h, cache):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    d_nexth_d_for_tan = (1 - torch.tanh(hidden)*torch.tanh(hidden))*dnext_h
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        N, H = prev_h.shape
        # Compute the activation vector a
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        if attn != None:
          WXattn = attn.mm(self.Wattn)
        else:
          WXattn = 0
        H = prev_h.shape[1]
        WxX = x.mm(self.Wx)
        WhH = prev_h.mm(self.Wh)
        a = WxX + WhH + self.b + WXattn
        ai = a[:, :H]
        af = a[:, H:2*H]
        ao = a[:, 2*H:3*H]
        ag = a[:, 3*H:4*H]
